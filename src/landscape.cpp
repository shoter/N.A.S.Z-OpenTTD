		...
		if (t != NULL) { ///< this if is required or scenario editor will crash with no towns on certain actions
			...
			switch(type) {
				...
				case MP_RAILWAY:  ///< don't give you more money on selling than you paid
					...
					cost.MultiplyCost(((t->cache.population / 5100) * _settings_game.ourSettings.clearLandscapePopulationMultiplier * (1536 / (distance  * _settings_game.ourSettings.clearLandscapeDistanceMultiplier + 1))) + 1);
					break;
				case MP_TREES:
